...
"""
  ...
  CALLS:
    - Python3 (known to work on version 3.5.2)
    - Matplotlib (known to work on version 2.0.2)
  ...
  NOTES:
    ...
    - Tested using Python 3.5.2 and Matplotlib 2.0.2
...
def short_help_message():
  ...
  print("USAGE:   [python3] read_weewxdata.py inputfile=inputfile start=date/time1 end=date/time2 plot_var='var1','var2',...,'varn' [--verbose] [--view_request_time] [debug=0|1] [verbose=0|1] [plot_var2='var1','var2',...,'varn'] ")
  print(" - Optional parameters are listed within square brackets ([]).")
  print("EXAMPLE: python3 read_weewxdata.py inputfile='weewx.sdb' start='2017-07-02T19:00:00' end='2017-07-05T19:00:00' plot_var='MinWindChill','HeatIndex','Dewpoint','OutTemp'")
...
for r in range(len(ADdays)):
  ...
  if (ADdays[r] < (currday+1.0+gmt_offset)):
    ...
    for v in range(len(variablelist)):
      # Check to see if the current variable value is not None
      if WxStationData[variablelist[v]][r] is not None:
